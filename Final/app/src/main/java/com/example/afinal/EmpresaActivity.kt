...
class EntrenadorActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(posicion>-1){
            var entrenador:Empresa= BddService.obtener_entrenador(posicion)
            var pokemon:Empleado= BddService.obtener_pokemon(posicion)
    ...
    fun ir_agregar_entrenador(posicion:Int){
        val intentExplicito= Intent(this, AgregarEmpresaActivity::class.java)
